...
/// @title OracleChainlinkMultiEfficientWithKeeper
/// @author Angle Core Team
/// @notice Abstract contract to build oracle contracts looking at Chainlink feeds
/// @dev The oracle is voluntarily paused for a few blocks after each oracle update
/// @dev This is an abstract contract that should be overriden with the correct addresses of the Chainlink feed
/// and the right amount of decimals
abstract contract OracleChainlinkMultiEfficientWithKeeper is ChainlinkUtilsWithKeeper {
    // =============================== Constants ===================================
    ...
    /// @param _pausingPeriod Period after each oracle update during which the feed is frozen
    /// for non whitelisted users
    /// @param _coreBorrow Contract handling access control
    ...
    constructor(
        ...
        if (address(_coreBorrow) == address(0) || address(_keeperRegistry) == address(0)) revert ZeroAddress();
    ...
    /// @notice Whether the Chainlink feeds should be paused after an update or not
    function _circuitChainlinkIsPausable() internal pure virtual returns (uint8[2] memory) {
        return [0, 1];
    }
}
