...
contract PoolManager is PoolManagerInternal, IPoolManagerFunctions {
  ...
  function deployCollateral(
    ...
    perpetualManager.deployCollateral(
      governorList,
      guardian,
      _feeManager,
      _oracle
    );
    _feeManager.deployCollateral(
      governorList,
      guardian,
  ...
  function removeGovernor(address _governor)
    external
    override
    onlyRole(STABLEMASTER_ROLE)
  {
  ...
  function setGuardian(address _guardian, address guardian)
    external
    override
    onlyRole(STABLEMASTER_ROLE)
  {
  ...
  function setFeeManager(IFeeManager _feeManager)
    external
    override
    onlyRole(STABLEMASTER_ROLE)
  {
  ...
  function estimatedAPR() external view returns (uint256 apr) {
    ...
    for (uint256 i = 0; i < strategyList.length; i++) {
      apr =
        apr +
        (strategies[strategyList[i]].debtRatio *
          IStrategy(strategyList[i]).estimatedAPR()) /
        BASE_PARAMS;
    }
    apr =
      (apr * slpData.interestsForSLPs * _getTotalAsset()) /
      sanRate /
      supply;
  }
  ...
  function report(
    ...
    if (target > params.totalStrategyDebt) {
      ...
      uint256 available = Math.min(
        target - params.totalStrategyDebt,
        _getBalance()
      );
  ...
  function recoverERC20(
    ...
    if (tokenAddress == address(token)) {
      ...
      ) = IStableMaster(stableMaster).collateralMap(
  ...
  function addStrategy(address strategy, uint256 _debtRatio)
    external
  ...
  function setSurplusConverter(address newSurplusConverter)
    external
  ...
  /// @param strategy The address of the `Strategy`
  function setStrategyEmergencyExit(address strategy)
    external
    onlyRole(GUARDIAN_ROLE)
  {
  ...
  function withdrawFromStrategy(IStrategy strategy, uint256 amount)
    external
    onlyRole(GUARDIAN_ROLE)
  {
  ...
  /// @notice @notice Allows to push interests revenue accumulated by the protocol to the `surplusConverter`
  /// to do buybacks or another form of redistribution to ANGLE or veANGLE token holders
